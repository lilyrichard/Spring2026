...
dir = Target + '_TI-R/DCS_3030_' + TS_case + '/'
...
# ----------------------------------------------------------------------------
fold_H = '/Users/lricha95/Library/CloudStorage/OneDrive-KennesawStateUniversity(2)/Research/mp_calc/data/H/'
fold_C = '/Users/lricha95/Library/CloudStorage/OneDrive-KennesawStateUniversity(2)/Research/mp_calc/data/C/'
cm = 1e7*nm
# get theta grid
...
# ----------------------------------------------------------------------------
#             R_ij distance filters for TI-IAM and IAM
# ----------------------------------------------------------------------------
R_filter_list = [4.0, 6.0, 8.0]
R_filter_label = ['Rlt4', 'Rlt6', 'Rlt8']
da = 0.02*np.pi
db = da
dc = da
...
def TI_IAM(index, i_E_max, target_w, R_filter=None):
    ...
    for i in range(0, len(atom)):
        ...
        for j in range(i+1, len(atom)):
            ...
            Rij_dist = np.linalg.norm(Rij)
            if R_filter is not None and Rij_dist >= R_filter:
                continue
    ...
    return MIT_target_tmp

def iso_IAM(i_E_max, R_filter=None):
    # ----------------------------------------------------------------------------
    ...
    for i in range(0, len(atom)):
        ...
        for j in range(i+1, len(atom)):
            ...
            if R_filter is not None and R12 >= R_filter:
                continue
            if atom[j] == 'C':
                f2 = f_C.copy()
            else:
...
for i_I in range(0, nI):
    F0 = np.sqrt(I_list[i_I]*1e14/I0)
    Up = (F0/(2.*omega))**2

    if __name__ == '__main__':
        print('Intensity is', I_list[i_I], 'e14 W/cm^2.')
        ...
        np.save( dir + 'TI_rate_' + str(np.round(I_list[i_I],decimals=2)) + 'e14_' + Target, target_w)
        ...
        # ----------------------------------------------------------------------
        #             Isotropic IAM with R_ij distance filters
        # ----------------------------------------------------------------------
        for filt_idx, R_filt in enumerate(R_filter_list):
            print(f'Applying R_ij filter to IAM: R < {R_filt}')
            MIT_target_iso = iso_IAM(i_E_max, R_filter=R_filt)
            data_iso = np.zeros((nE,2))
            data_iso[:, 0] = nrg/eV
            data_iso[:i_E_max, 1] = MIT_target_iso[:i_E_max] #+ I_A[:i_E_max] save just MIT
            np.savetxt(dir + Target + '_IAM_' + R_filter_label[filt_idx] + '_' + str(np.round(I_list[i_I],decimals=2)) + 'e14.dat', data_iso)
            iam_plot_data.append((R_filter_label[filt_idx], data_iso[:,0].copy(), data_iso[:,1].copy()))
            print(f'Saved {Target}_IAM_{R_filter_label[filt_idx]}_{np.round(I_list[i_I],decimals=2)}e14.dat')

        # ----------------------------------------------------------------------
        #             TI-IAM with R_ij distance filters
        # ----------------------------------------------------------------------
        for filt_idx, R_filt in enumerate(R_filter_list):
            print(f'Applying R_ij filter: R < {R_filt}')
            index = np.arange(0,Nc*Nb*Na,1, dtype=np.int_)

            MIT_target = np.zeros((np.size(alpha), np.size(beta),
                        np.size(gamma), i_E_max))
            
            func = partial(TI_IAM, i_E_max = i_E_max, target_w = target_w, R_filter = R_filt)
            with mp.Pool(ncore) as p:
                MIT_target_raw = p.map(func, index)
            for i in range(0, np.size(index)):
                ia, ib, ic = index_to_angs(i)
                MIT_target[ia, ib, ic, :] = MIT_target_raw[i]
            ...
            #data[:i_E_max, 2] = I_A[:i_E_max] * norm_target + MIT_target_avg
            np.savetxt(dir + Target + '_TI-IAM_' + R_filter_label[filt_idx] + '_' + str(np.round(I_list[i_I],decimals=2)) + 'e14.dat', data)
            tiam_plot_data.append((R_filter_label[filt_idx], data[:,0].copy(), data[:,1].copy()))
            print(f'Saved {Target}_TI-IAM_{R_filter_label[filt_idx]}_{np.round(I_list[i_I],decimals=2)}e14.dat')

        # ----------------------------------------------------------------------
        #             Plot IAM and TI-IAM filter results
        # ----------------------------------------------------------------------
        print('Creating comparison figure...')
        fig, axes = plt.subplots(1, 2, figsize=(12, 5), sharex=True, sharey=True)
        ymin = np.inf
        ymax = -np.inf
        for _, _, yvals in iam_plot_data + tiam_plot_data:
            if yvals[:i_E_max].size > 0:
                ymin = min(ymin, np.min(yvals[:i_E_max]))
                ymax = max(ymax, np.max(yvals[:i_E_max]))
        if ymin == np.inf:
            ymin, ymax = 0, 1
        axes[0].set_xlim(nrg[0]/eV, nrg[i_E_max-1]/eV)
        axes[0].set_ylim(ymin, ymax)
        axes[1].set_xlim(nrg[0]/eV, nrg[i_E_max-1]/eV)
        axes[1].set_ylim(ymin, ymax)

        for label, xvals, yvals in iam_plot_data:
            axes[0].plot(xvals[:i_E_max], yvals[:i_E_max], linewidth=1.5)
            axes[0].annotate(label, xy=(xvals[i_E_max-1], yvals[i_E_max-1]), xytext=(-5, 0), textcoords='offset points', ha='right', va='center', fontsize=8)
        axes[0].set_title('IAM')
        axes[0].set_xlabel('Energy (eV)')
        axes[0].set_ylabel('MIT')
        axes[0].grid(True, linestyle=':', alpha=0.5)

        for label, xvals, yvals in tiam_plot_data:
            axes[1].plot(xvals[:i_E_max], yvals[:i_E_max], linewidth=1.5)
            axes[1].annotate(label, xy=(xvals[i_E_max-1], yvals[i_E_max-1]), xytext=(-5, 0), textcoords='offset points', ha='right', va='center', fontsize=8)
        axes[1].set_title('TI-IAM')
        axes[1].set_xlabel('Energy (eV)')
        axes[1].grid(True, linestyle=':', alpha=0.5)

        fig.tight_layout()
        fig.savefig(Target + '_MIT_BL_Filters.pdf')
        print(f'Saved figure {Target + "_MIT_BL_Filters.pdf"}')
